...
def char2mozzi(infile, outfile, tablename, samplerate):
	...
	print ("opened " + infile)
	uint8_tstoread = os.path.getsize(os.path.expanduser(infile)) / 2
	##print uint8_tstoread
	valuesfromfile = array.array('h') # array of signed int8_t ints
	try:
	...
	outstring = 'const int16_t __attribute__((section(".progmem.data"))) ' + tablename + '_DATA [] = {'
	try:
